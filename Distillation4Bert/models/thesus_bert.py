...
from transformers.modeling_bert import BertConfig, BertLayer, BertLayerNorm, BertEmbeddings, \
    BertPooler, BertPreTrainingHeads, BertOnlyMLMHead
from transformers.modeling_bert import BERT_PRETRAINED_MODEL_ARCHIVE_MAP, load_tf_weights_in_bert
import logging
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.scc_n_layer = config.scc_n_layer if config.scc_n_layer > 0 else 6
...
class TheseusBertPreTrainedModel(PreTrainedModel):
...
class TheseusBertModel(TheseusBertPreTrainedModel):
    def __init__(self, config):
...
class TheseusBertForPreTraining(TheseusBertPreTrainedModel):
    def __init__(self, config):
        super(TheseusBertForPreTraining, self).__init__(config)

        self.bert = TheseusBertModel(config)
...
class TheseusBertForMaskedLM(TheseusBertPreTrainedModel):
    def __init__(self, config):
        super(TheseusBertForMaskedLM, self).__init__(config)
...
class TheseusBertForSequenceClassification(TheseusBertPreTrainedModel):
    def __init__(self, config):
        super(TheseusBertForSequenceClassification, self).__init__(config)
