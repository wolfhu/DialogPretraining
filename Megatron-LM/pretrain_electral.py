...
def get_optimizer(model, args):
    ...
    discriminitor_ouput_layer = model.model.d_outputlayer.dense
    param_groups = []
...
def forward_step(data_iterator, model, args, timers, data = None):
    ...
    # Forward model.
    lm_loss, nsp_loss = model(tokens, types, 1-padding_mask, lm_labels,
...
def evaluate_and_print_results(prefix, data_iterator, model,
    ...
    val_loss = lm_loss + factor * nsp_loss
    print_rank_0('-' * 100)
