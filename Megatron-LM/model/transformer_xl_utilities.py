...
# def local_log_softmax_default(input_logits):
#         # Copy so the input remains unchanged.
#         logits = input_logits.clone()
#         logits_max = torch.max(logits, dim=-1)[0]
#         # Subtract the maximum value.
#         logits.sub_(logits_max.unsqueeze(dim=-1))
#         exp_logits = logits.exp()
#         sum_exp_logits = exp_logits.sum(dim=-1, keepdim=True)
#         # Loss = logits - log(sum(exp(logits)))
#         loss = logits - torch.log(sum_exp_logits)
#         return loss

def local_log_softmax(input_logits, dim):
        # Copy so the input remains unchanged.
        logits = input_logits.clone()
...
class ProjectedAdaptiveLogSoftmax(nn.Module):
    def __init__(self, n_token, d_embed, d_proj, cutoffs, div_val=1,
        ...
        if div_val == 1:
            ...
            self.out_layers.append(nn.Linear(d_embed, n_token, False))
        else:
            for i in range(len(self.cutoffs)):
    ...
    def forward(self, hidden, target, keep_order=False):
        ...
        if self.n_clusters == 0:
            ...
            # nll = -local_log_softmax_default(logit) \
            #         .gather(1, target.unsqueeze(1)).squeeze(1)
            nll = -local_log_softmax(logit, dim=-1) \
                    .gather(1, target.unsqueeze(1)).squeeze(1)
        else:
            if target is None:
                # TODO: @kaizh This is not implemented
            ...
            head_logprob = local_log_softmax(head_logit, dim=1)
            ...
            for i in range(len(cutoff_values) - 1):
                ...
                if i == 0:
                    ...
                    tail_logprob_i = local_log_softmax(tail_logit_i, dim=1)
