...
class Electra_OuputLayer(nn.Module):
    def __init__(self, nh_size, output_size, activation=nn.Sigmoid()):
        super(Electra_OuputLayer, self).__init__()
...
class BertForElectra(PreTrainedBertModel):
    """BERT model with the masked language modeling head.
    This module comprises the BERT model followed by the masked language modeling head.
    ...
    """
    def __init__(self, config):
        super(BertForElectra, self).__init__(config)

        self.embedding = BertEmbeddings(config)

        g_config = config
        g_config.num_hidden_layers = 2
        self.g_encoder = BertEncoder(g_config)
        d_config = config
        d_config.num_hidden_layers = 4
        self.d_encoder = BertEncoder(d_config)
        self.d_outputlayer = Electra_OuputLayer(config.hidden_size, 1)
        self.cls = BertOnlyMLMHead(config, self.embedding.word_embeddings.weight)
        ...
        # self.apply(self.init_bert_weights)
        #
    def generator_encoder(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, checkpoint_activations=False):
        if attention_mask is None:
            attention_mask =  torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        ...
        extended_attention_mask = extended_attention_mask.to(
            dtype=next(self.g_encoder.parameters()).dtype)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0

        embeddings = self.embedding(input_ids, None)
        # print("g_encoder embeddings layer type", embeddings.type())
        encoder_layers = self.g_encoder(embeddings, extended_attention_mask,
                                        output_all_encoded_layers= output_all_encoded_layers,
                                        checkpoint_activations = checkpoint_activations)

        return encoder_layers

    def discriminitor_encoder(self, inputs_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, checkpoint_activations=False):
        if attention_mask is None:
        ...
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(
            dtype=next(self.d_encoder.parameters()).dtype)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0

        embeddings = self.embedding(inputs_ids, None)
        # print("d_encoder embeddings layer type", embeddings.type())
        encoder_layers = self.d_encoder(embeddings, extended_attention_mask,
                                        output_all_encoded_layers= output_all_encoded_layers,
                                        checkpoint_activations = checkpoint_activations)
        # print("d_output layer type", encoder_layers[-1].type())
        output = self.d_outputlayer(encoder_layers[-1])

        return output

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, checkpoint_activations=False, factor = 50):
        sequence_output = self.generator_encoder(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False,
                                                    checkpoint_activations= checkpoint_activations)
        ...
        generated = torch.argmax(F.softmax(sequence_output), -1)
        generated_mask = masked_lm_labels != -1
        # logger.info("generated_mask type is", generated_mask.type())
        generated_mask = torch._cast_Long(generated_mask)
        input_mask = torch._cast_Long(masked_lm_labels == -1)

        replaced = generated * generated_mask
        _input = input_ids * input_mask
        replaced = replaced + _input

        discriminitor_label = torch._cast_Long(replaced == input_ids)
        # print("corrupted type is ", replaced.type())
        discriminitor_output = self.discriminitor_encoder(replaced, token_type_ids, attention_mask,
                                                          output_all_encoded_layers=False)
        # print("output.type", discriminitor_output.type())
        if discriminitor_label is not None:
            loss_fct = nn.BCELoss()
            discriminitor_loss = loss_fct(torch.squeeze(discriminitor_output), discriminitor_label.float())

        return masked_lm_loss, discriminitor_loss
