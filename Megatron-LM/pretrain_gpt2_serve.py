...
def get_config_value(key, default):
    config_path = '/root/config/finetune_config'
    ...
    return load_dict[key] if key in load_dict else default


def set_infer_config(args):
    TRAIN_DATA = '/mnt/azure/kazh/corpus/' + get_config_value('train_data', 'qingchun.json')
    CHECKPOINT_LOAD = '/mnt/azure/' + get_config_value('checkpoint_load', 'basemodel-20200706')
    CHECKPOINT_SAVE = '/mnt/azure/kazh/model_checkpoints/' + get_config_value('checkpoint_save', 'model_qingchun')
    BATCH_SIZE = get_config_value('batch_size', 40)
    SEQ_LENGTH = get_config_value('seq_length', 1024)
    MAX_POSITION_EMBEDDINGS = get_config_value('max_position_embeddings', 1024)
    SAVE_INTERVAL = get_config_value('save_interval', 1000)
    TRAIN_ITERS = get_config_value('train_iters', 5000)
    LR = get_config_value('lr', 1.5e-4)
    LR_DECAY_STYLE = get_config_value('lr_decay_style', 'cosine')
    SPLIT = get_config_value('split', '949,50,1')

    args.train_data = TRAIN_DATA
    args.load = CHECKPOINT_LOAD
    args.save = CHECKPOINT_SAVE
    ...
    args.text_key = 'text'
    args.lr = LR
    args.lr_decay_style = LR_DECAY_STYLE
    args.lazy_loader = True
    args.loose_json = True
    args.reset_position_ids = True
    args.reset_attention_mask = True
    args.eod_mask_loss = True
