...
def get_configmap_value(key, defaultvalue):
    config = '/root/config/' + key
    if not os.path.exists(config):
        return defaultvalue
    with open(config) as f:
        return f.read()

def set_infer_config(args):
    CHECKPOINT_PATH = '/mnt/azure/' + get_configmap_value('CheckPointPath', 'basemodel-20200706')
    MPSIZE = int(get_configmap_value('MPSize', '1'))
    NLAYERS = int(get_configmap_value('NLayers','24'))
    NHIDDEN = int(get_configmap_value('NHidden', '1024'))
    NATT = int(get_configmap_value('NATT', '16'))
    MAXSEQLEN = int(get_configmap_value('MaxSeqLen', '960'))
    TEMP = int(get_configmap_value('TEMP', '1'))
    TOPK = int(get_configmap_value('TOPK', '50'))
    TOPP = float(get_configmap_value('TOPP', '0.95'))
    SERVERPORT = int(get_configmap_value('ServerPort', '3322'))
    FP16 = bool(get_configmap_value('FP16', 'True'))
    SOCKETMODE = bool(get_configmap_value('SocketMode', 'True'))
    ...
    args.batch_size  = 1 
    args.tokenizer_type =  'GPT2WordPieceTokenizer'
    args.tokenizer_model_type  = 'gpt2-large-chinese-refine' 
    args.fp16 = FP16
    args.cache_dir = 'cache'
    args.seq_length = MAXSEQLEN 
...
def parse_request_socket(request_data):
...
def client_join(data):
    ...
    room = data['room']
    join_room(room)
    socketio.emit('join_msg', username + ' has entered the room.', room=room)
...
def client_leave(data):
    ...
    room = data['room']
    leave_room(room)
    socketio.emit('leave_msg', username + ' has leaved the room.', room=room)

@socketio.on('generate-socket')
def generate_(req_json):
    args = ARGS_INSTANCE
    tokenizer = TOKENIZER_INSTANCE
    model = MODEL_INSTANCE
    
    req_json = parse_request_socket(req_json)
    if not req_json:
        return
    
    room = req_json['room']
    if args.world_size != 1:
        socketio.emit("error", "Only support world size == 1", room = room)
        return
    
    input_text = req_json['input'] if 'input' in req_json else  None
    if not input_text:
        socketio.emit("error", "Request input is empty", room = room)
        return

    end_token = req_json['eos'] if 'eos' in req_json else  'eos'
    end_token_id = tokenizer.get_command(end_token).Id

    out_seq_length = req_json['out_seq_length'] if 'out_seq_length' in req_json else args.out_seq_length

    temperature = req_json['temperature'] if 'temperature' in req_json else args.temperature
    top_k = req_json['top_k']
    top_p = req_json['top_p']

    mem_length = req_json['mem_length']
    # print('mem length', mem_length)
    max_mem_length = max(args.seq_length // 2, args.seq_length - 100)
    # print("max mem length", max_mem_length)
    if mem_length >= max_mem_length:
        socketio.emit("error", "mem_lenght must less than {}, but we get {}".format(max_mem_length, mem_length), room = room)
        return

    if out_seq_length > args.seq_length and mem_length <= 0:
        socketio.emit("error", "When mem_length <= 0, out_seq_length must less than or equal to {}. However, we get out_seq_length={}, mem_length={}. Plese set mem_length value in your request, if you want to get a result length > {}, ".format(args.seq_length, out_seq_length, mem_length, args.seq_length), room = room)
        return

    do_convert_sep = req_json['do_convert_sep']
    ...
    if len(context_tokens) >= args.seq_length:
        socketio.emit("error",  "input bpe tokens length must less than {}, but we get {}".format(args.seq_length, len(context_tokens)), room = room)
        return
    ...
    while True:
        with torch.no_grad():
            ...
            for counter, decode_tokens in enumerate(token_stream):
                ...
                tokens = tokenizer.text_tokenizer.convert_ids_to_tokens(decode_tokens)
                tokens = format_zh_bpe_result(tokens)
                socketio.emit("response", tokens[-1], room = room)
            output_tokens.extend(decode_tokens)
            if mem_length <= 0 or len(output_tokens) >= out_seq_length or output_tokens[-1] == end_token_id:
                break
...
def main():
    ...
    print("Prepare for the request. Init model and args")
...
if __name__ == "__main__":
    os.environ["CUDA_VISIBLE_DEVICES"] = get_configmap_value("cuda_device_id", '0')
    main()
    args = ARGS_INSTANCE
    if args.socket_mode:
        socketio.run(app, debug=False, host='0.0.0.0', port = args.server_port)
    else:
        app.run(host='0.0.0.0', port=args.server_port)
