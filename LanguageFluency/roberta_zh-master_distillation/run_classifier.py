...
class IsinstanceProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if set_type == 'test':
              ...
              label = tokenization.convert_to_unicode('correct')
            else:
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, per_example_loss, logits, output_layer) = create_model(
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          predictions={"output_layer": output_layer, "logit": logits},
          scaffold_fn=scaffold_fn)
    return output_spec
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    output_predict_file_logit = os.path.join(FLAGS.output_dir, "test_results_logit.tsv")
    output_predict_file_output_layer = os.path.join(FLAGS.output_dir, "test_results_output_layer.tsv")
    with tf.gfile.GFile(output_predict_file_logit, "w") as writer_logit:
      with tf.gfile.GFile(output_predict_file_output_layer, "w") as writer_output_layer:
        num_written_lines = 0
        ...
        for (i, prediction) in enumerate(result):
          ...
          if i >= num_actual_predict_examples:
            break
          output_line_logit = "\t".join(str(class_probability) for class_probability in logit) + "\n"
          writer_logit.write(output_line_logit)
          
          output_line_output_layer = "\t".join(str(class_probability) for class_probability in output_layer) + "\n"
          #writer_output_layer.write(output_line_output_layer)
          num_written_lines += 1
